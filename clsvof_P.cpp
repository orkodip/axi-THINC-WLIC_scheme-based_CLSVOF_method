...
class CLSVOF:public MBASE
{
	...
	double **F_act,**Phi_act;	//actual volume fraction and level set fields
	...
	void adv_X();	//solve the advection equations (r direction followed by y direction)
	void adv_Y();	//solve the advection equations (y direction followed by r direction)
	...
	public:
			CLSVOF(); ~CLSVOF();
			void ini(double xc,double yc,double a,double b);	//initialization of the filed variables
			void solve(int n);	//CLSVOF advection algorithm
			double adv_err();	//calculate the advection error
			void mass_err();	//calculate mass error (here volume error)
...
void CLSVOF::ini(double xc,double yc,double a,double b)
{
	...
	INI ms(Xm,Ym,CX,CY,F_act,Phi_act,xc,yc,beta_x,beta_y,a);	//initial F and Phi fields
...
void CLSVOF::adv_X()
{
	...
	for(int i=1;i<=I;i++)
	{
		for(int j=1;j<=J;j++)
		{
			...
			F_adv[0]=F_adv[1];	//updation for the next cell
			LS_adv[0]=LS_adv[1];
			if(abs(F[j][i])<=TRUNC) F[j][i]=0.0;	//empty cell
			else if(abs(F[j][i]-1.0)<=TRUNC) F[j][i]=1.0;	//completely filled cell
			if(F[j][i]<0.0) F[j][i]=0.0;
			else if(F[j][i]>1.0) F[j][i]=1.0;
		}
	}
	updt_ghost(Phi);
}
void CLSVOF::adv_Y()
{
	...
	for(int i=1;i<=I;i++)
	{
		for(int j=1;j<=J;j++)
		{
			...
			{
				F_adv[1]=vol_frac_flux(1,i,j,F,Phi,v_NS[j][i]);
	...
	for(int j=1;j<=J;j++)
	{
		for(int i=1;i<=I;i++)
		{
			...
			{
				F_adv[1]=vol_frac_flux(0,i,j,Ft,Phit,u_EW[j][i]);
